...
if __name__ == '__main__':
    ...
    # opt = parser.parse()
    cfg = load_config("./config/eval_momaskplus_hml.yaml")
    fixseed(cfg.seed)
    ...
    mask_trans_cfg = load_config(pjoin(cfg.root_ckpt_dir, cfg.data.name, 'momask_plus', cfg.mask_trans_name, 'train_momaskplus_hml.yaml'))
    # res_trans_cfg = load_config(pjoin(cfg.root_ckpt_dir, cfg.data.name, 'res_trans', cfg.res_trans_name, 'res_transformer.yaml'))

    # print(mask_trans_cfg.vq_name, res_trans_cfg.vq_name)
    # assert mask_trans_cfg.vq_name == res_trans_cfg.vq_name

    vq_cfg = load_config(pjoin(cfg.root_ckpt_dir, cfg.data.name, 'vq', mask_trans_cfg.vq_name, 'residual_vqvae_hml.yaml'))
    ...
    val_cid_split_file = pjoin(cfg.data.root_dir, 'test.txt')
                ...
                for i in range(cfg.repeat_time):
                    ...
                    with torch.no_grad():
                        best_fid, best_div, Rprecision, best_matching, best_mm = (
                            eval_t2m.evaluation_momask_plus_hml(
                                eval_loader,
                                vq_model,
                                t2m_transformer,
                                i,
